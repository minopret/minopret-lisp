...
(eval           (label eval (lambda (x a) (cond
    ...
    ((atom (car x)) (cond
        ...
        ((eq (car x) 'cond) (
            (lambda (pairs) (cond
                ((null pairs) '())  ; standard? 
                ((eval (caar pairs) a) (eval (cadar pairs) a))
                ( t (eval (cons 'cond (cdr pairs)) a)) ))
            (cdr x) ))
            ...
            (label evlis-rec (lambda (x a r) (cond
                ((null x) (
                    (label evlis-rev (lambda (f r) (cond
                        ((null r) f)
                        ( t (evlis-rev (cons (car r) f) (cdr r))) )))
                    () r ))
                ( t (evlis-rec (cdr x) a (cons (eval (car x) a) r))) )))
            (cdr x) a () ))
         a )) )
    ((eq (caar x) 'label) (eval
        (cons (caddar x) (cdr x))
